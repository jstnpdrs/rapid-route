...
<template>
  <div class="flex items-center justify-between">
    <div class="flex flex-1 items-center space-x-2">
      ...
      <Input placeholder="Search..." :model-value="(table.getColumn('name')?.getFilterValue() as string) ?? ''"
        class="h-8 w-[150px] lg:w-[250px]" @input="table.getColumn('name')?.setFilterValue($event.target.value)" />

      <DataTableFacetedFilter v-for="filter in props.filters" :column="table.getColumn(filter.column)"
        :title="filter.title" :options="filter.options" />
      <Button v-if="isFiltered" variant="ghost" class="h-8 px-2 lg:px-3" @click="table.resetColumnFilters()">
        Reset
        <X class="ml-2 h-4 w-4" />
      </Button>
    </div>
    <Button v-if="newRecordLink" variant="default" class="ml-auto h-8 px-2 lg:px-3"
      @click="router.visit(newRecordLink)">
      {{ createButtonLabel }}
    </Button>
  </div>
</template>
